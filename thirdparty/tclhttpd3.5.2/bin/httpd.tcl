...
if {[info exists ::bowwow::version]} {
    append startup "$::bowwow::name $::bowwow::version\n"
}
append startup "Server started on port $Config(port)\n"
...
if {[info exists tk_version] && $Config(gui)} {
    ...
    SrvUI_Init "$::bowwow::name $::bowwow::version"
} else {
    append startup "Enter Ctrl-C to exit..."
}
